...
def resolve(url: str) -> str:
    command = [
        ...
        "--retries", "3", "--format", "best[protocol^=m3u8]/best", "--get-url", url,
    ]
...
def launch(slug: str, stream_url: str) -> subprocess.Popen:
    directory = output_dir(slug)
    command = [
        "ffmpeg", "-hide_banner", "-loglevel", "warning", "-nostdin",
        "-reconnect", "1", "-reconnect_streamed", "1", "-reconnect_delay_max", "10",
        "-i", stream_url, "-map", "0:v:0", "-map", "0:a:0?", "-c", "copy",
        "-f", "hls", "-hls_time", "4", "-hls_list_size", "12",
        "-hls_delete_threshold", "6",
        "-hls_flags", "delete_segments+append_list+omit_endlist+independent_segments",
        "-hls_segment_filename", str(directory / "segment_%09d.ts"),
        str(directory / "index.m3u8"),
    ]
    return subprocess.Popen(command, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True, start_new_session=True)
...
def start(slug: str):
    ...
    with state.lock:
        ...
        clean(slug)
    try:
        stream_url = resolve(CHANNELS[slug])
        with state.lock:
            process = launch(slug, stream_url)
            state.process = process
        threading.Thread(target=consume_errors, args=(slug, process), daemon=True).start()
...
def touch_and_start(slug: str):
    ...
    if should_start:
        threading.Thread(target=start, args=(slug,), daemon=True).start()
    ...
    while True:
        ...
        for slug, state in states.items():
            ...
            if is_running and idle >= IDLE_TIMEOUT:
                stop(slug, f"{int(idle)} segundos sin solicitudes")
        time.sleep(10)
